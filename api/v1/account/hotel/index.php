<?php
    ...
    $reqprep = $connexion->prepare('SELECT ID, NAME FROM HOTEL WHERE MANAGER =
        (SELECT USER FROM SESSION WHERE TOKEN = :token) LIMIT 30 OFFSET :off;');
    ...
    if ($res == false) {
        $response['success'] = array('No hotel for this user.');
        echo json_encode($response);
        die();   
    }
