<?php
    ...
    unset($valid['next']);
    $req = 'SELECT ID, FNAME, LNAME, TYPE, PERM FROM USER WHERE (SELECT TYPE 
        FROM USER WHERE ID = (SELECT USER FROM SESSION WHERE TOKEN = :token)) 
        = "ADMIN" AND PERM < (SELECT PERM FROM USER WHERE ID = (SELECT USER 
        FROM SESSION WHERE TOKEN = :token))';
    if (isset($valid['name'])) {
        $req .= ' AND FNAME LIKE CONCAT(\'%\', :name, \'%\') OR LNAME LIKE 
        CONCAT(\'%\', :name, \'%\')';
    }
